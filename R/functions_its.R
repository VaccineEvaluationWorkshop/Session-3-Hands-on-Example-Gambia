step_func <- function(ds,
  ...
  ds$post1<-as.numeric((ds$time_index >= ds$time_index[time_points == post_period1[1]] & ds$time_index < ds$time_index[time_points == post_period1[2]]) )
  ds$post2<-as.numeric((ds$time_index >= ds$time_index[time_points == post_period2[1]] & ds$time_index < ds$time_index[time_points == post_period2[2]]) )
  ds$post3<-as.numeric(ds$time_index >= ds$time_index[time_points == post_period3[1]] )
  ...
  rr.out <- list('rr.q.post' = rr.q.post, 'aic1'=aic1,'outcome'=ds[,outcome_name],
                 ...
                 'rr.q.t'=rr.q.t,'overdispersion'=overdispersion)
  return(rr.out)
}
...
spline_func <- function(ds,
  ...
  ds$post1<-ds$time_index - ds$time_index[time_points == post_period1[1]] 
  ds$post1[ds$post1<0]<-0
  ds$post2<-ds$time_index - ds$time_index[time_points == post_period2[1]] 
  ds$post2[ds$post2<0]<-0
  ds$post3<-ds$time_index - ds$time_index[time_points == post_period3[1]] 
